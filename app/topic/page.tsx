...
function findArticlesDeep(node: any, result: any[] = []) {
...
function collectSentences(node: any, result: string[] = []) {
  if (!node) return result;
...
function extractCaption(article: any) {
...
function renderParagraph(node: any, keyPrefix = "") {
  ...
  return (
    <Box key={keyPrefix} sx={{ ml: 2, mt: 1 }}>
      {num ? (
        <Typography sx={{ fontWeight: "bold", mb: 1 }}>（{num}）</Typography>
      ) : null}
